...
public class HangFrame extends javax.swing.JFrame {
    ...
    public HangFrame() {
        ...
        private void setTotalPage(List<Hang> list) {
    ...
    private void btnUpdate() {
        ...
        Hang hang = getTable();
        hang.setName(getForm().getName());
        int row = repository.update(hang);
        String thongBao = "";
        if (row == 1) {
    ...
    private void search() {
        ...
        List<Hang> listSearch = new ArrayList<>();
        for (Hang hang : repository.selectAll()) {
            if (hang.getMa().toLowerCase().contains(search) || hang.getName().toLowerCase().contains(search)) {
                listSearch.add(hang);
            }
        }
        if (totalPage == 1) {
