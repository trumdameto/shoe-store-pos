...
public class KichCoFrame extends javax.swing.JFrame {
    ...
    public KichCoFrame() {
        ...
        private void setTotalPage(List<KichCo> list) {
    ...
    private void btnUpdate() {
        ...
        KichCo kichCo = getTable();
        kichCo.setName(getForm().getName());
        int row = repository.update(kichCo);
        String thongBao = "";
        if (row == 1) {
    ...
    private void search() {
        ...
        List<KichCo> listSearch = new ArrayList<>();
        for (KichCo kichCo : repository.selectAll()) {
            if (kichCo.getMa().toLowerCase().contains(search) || kichCo.getName().toLowerCase().contains(search)) {
                listSearch.add(kichCo);
            }
        }
        if (totalPage == 1) {
            currentPage = 1;
