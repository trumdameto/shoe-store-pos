...
public class KieuDangFrame extends javax.swing.JFrame {
    ...
    public KieuDangFrame() {
        ...
        private void setTotalPage(List<KieuDang> list) {
    ...
    private void btnUpdate() {
        ...
        KieuDang kieuDang = getTable();
        kieuDang.setName(getForm().getName());
        int row = repository.update(kieuDang);
        String thongBao = "";
        if (row == 1) {
    ...
    private void search() {
        ...
        List<KieuDang> listSearch = new ArrayList<>();
        for (KieuDang kieuDang : repository.selectAll()) {
            if (kieuDang.getMa().toLowerCase().contains(search) || kieuDang.getName().toLowerCase().contains(search)) {
                listSearch.add(kieuDang);
            }
        }
        if (totalPage == 1) {
            currentPage = 1;
