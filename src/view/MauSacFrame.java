...
public class MauSacFrame extends javax.swing.JFrame {
    ...
    public MauSacFrame() {
        ...
        private void setTotalPage(List<MauSac> list) {
    ...
    private void btnUpdate() {
        ...
        MauSac mauSac = getTable();
        mauSac.setName(getForm().getName());
        int row = repository.update(mauSac);
        String thongBao = "";
        if (row == 1) {
    ...
    private void search() {
        ...
        for (MauSac mauSac : repository.selectAll()) {
            if (mauSac.getMa().toLowerCase().contains(search) || mauSac.getName().toLowerCase().contains(search)) {
                listSearch.add(mauSac);
            }
        }
        if (totalPage == 1) {
            currentPage = 1;
